...
namespace igLibrary.Core
{
    public class igIGZLoader
    {
        ...
        public void Read(igObjectDirectory dir, bool readDependancies)
        {
            ...
            {
                ParseSections();
                ProcessFixupSections(dir, numFixups);                                            // numFixups and _platform get set in this function in ssa
                ReadObjects();
            }
            else
            {
        ...
        {
            int sectionCount = 0;
            uint memPoolOffsetStart = (_version >= 7) ? 0x224u : 0x56Cu;
            for (int i = 0; i < 0x20; i++)
            {
                uint offset = 0;
                uint memPoolNameOffset = 0;
                uint length = 0;
                uint alignment = 0;
                if (_version >= 7)
                {
                    _stream.Seek(0x14 + 0x10 * i);
                    memPoolNameOffset = _stream.ReadUInt32();
                    offset = _stream.ReadUInt32();
                    length = _stream.ReadUInt32();
                    alignment = _stream.ReadUInt32();
                }
                else
                {
                    _stream.Seek(0xC + 0x10 * i);
                    memPoolNameOffset = _stream.ReadUInt32();
                    offset = _stream.ReadUInt32();
                    length = _stream.ReadUInt32();
                    alignment = _stream.ReadUInt32();
                ...
                _stream.Seek(memPoolOffsetStart + memPoolNameOffset);
        ...
        {
            uint bytesProcessed = (_version >= 7) ? 0x0u : 0x1Cu;
            if (_version < 7)
            {
                _stream.Seek(_fixups + 0x8);
                igMetaEnum platformEnum = igArkCore.GetMetaEnum("IG_CORE_PLATFORM");
                _platform = (IG_CORE_PLATFORM)platformEnum.GetEnumFromValue(_stream.ReadInt16());
                _stream.Seek(0x06, SeekOrigin.Current);
                numFixups = _stream.ReadUInt32();
            }
            for (int i = 0; i < numFixups; i++)
            {
                ...
                if (_version < 7) _stream.Seek(0x08, SeekOrigin.Current);
                ...
                switch (magic)
                {
                    ...
                    case 0x0F:                          //RPID
                        ...
                        break;
                    case 0x54545352:                            //RSTT //doesnt exist in SSA
                    ...
                    case 0x10:                          //RNEX
                        ...
                        break;
                    case 0x444E4852:                            //RHND // doesnt exist in SSA
