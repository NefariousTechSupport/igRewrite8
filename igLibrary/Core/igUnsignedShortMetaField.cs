...
namespace igLibrary.Core
{
	...
	{
		public static igUnsignedShortMetaField _MetaField { get; private set; } = new igUnsignedShortMetaField();
		...
		{
			if (!ushort.TryParse(input, Localisation.kENNumberStyles, Localisation.kENCultureInfo, out ushort buffer))
			{
				return false;
			}
			target = buffer;
