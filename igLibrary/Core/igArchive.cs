...
namespace igLibrary.Core
{
    ...
    public class igArchive : igStorageDevice
    {
        ...
        public bool _ownsFileDescriptor;
        ...
        public bool _needNpDrmDecrypt;
        public bool _caseInsensitiveHash;
        ...
        public void Open(string filePath, igBlockingType blockingType)
        {
            ...
            switch (_archiveHeader._version)
            {
                ...
                case 0x04:
                    _archiveHeader._tocSize = sh.ReadUInt32();
                    _archiveHeader._numFiles = sh.ReadUInt32();
                    _archiveHeader._sectorSize = 0x0800;
                    _archiveHeader._hashSearchDivider = sh.ReadUInt32();
                    _archiveHeader._hashSearchSlop = sh.ReadUInt32();
                    _archiveHeader._nameTableOffset = sh.ReadUInt32();
                    _archiveHeader._nameTableSize = sh.ReadUInt32();
                    _archiveHeader._numLargeFileBlocks = sh.ReadUInt32();
                    _archiveHeader._numMediumFileBlocks = sh.ReadUInt32();
                    _archiveHeader._numSmallFileBlocks = sh.ReadUInt32();
            ...
            for (int i = 0; i < _archiveHeader._numFiles; i++)
            {
                ...
                if (_archiveHeader._version == 0xb)
                {
                ...
                else if (_archiveHeader._version == 0x4)
                {
        ...
        public void Save(string filePath)
        {
            ...
            switch (_archiveHeader._version)
            {
                ...
                case 0x04:
                    ...
                    sh.WriteUInt32(_archiveHeader._hashSearchDivider);
                    sh.WriteUInt32(_archiveHeader._hashSearchSlop);
                    sh.WriteUInt32((uint)_archiveHeader._nameTableOffset);
                    sh.WriteUInt32(_archiveHeader._nameTableSize);
                    sh.WriteUInt32(_archiveHeader._numLargeFileBlocks);
                    sh.WriteUInt32(_archiveHeader._numMediumFileBlocks);
                    sh.WriteUInt32(_archiveHeader._numSmallFileBlocks);
...
#pragma warning restore CS8602
        ...
        public void Decompress(FileInfo fileInfo, Stream dst)
        {
            ...
            CompressionType type = (_archiveHeader._version == 0x4) ? (CompressionType)((fileInfo._blockIndex >> 28) + 1) : (CompressionType)(fileInfo._blockIndex >> 28);
            byte[]? lzmaProps = null; // ^ I have no idea how to find the SSA compressiontype (it's not a metaenum there) so i'm doing this dumb workaround that works for the most common type /bone
            ...
            for (int i = 0; i < fileInfo._blocks.Length; i++)
            {
                ...
                using StreamHelper streamtest = new StreamHelper(fileInfo._compressedData);
                streamtest.Seek(offset, SeekOrigin.Begin);
                StreamHelper.Endianness compressSizeEndianness = _archiveHeader._version <= 0x04 ? StreamHelper.Endianness.Big : StreamHelper.Endianness.Little;
                uint compressedSize = streamtest.ReadUInt16(compressSizeEndianness);
                offset += 2;
