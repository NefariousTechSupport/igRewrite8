namespace igLibrary.Gfx.GX2Utils
{
    public class TextureHelper
    {
        public static Tuple<uint, uint> GetCurrentMipSize(uint width, uint height, uint blkWidth, uint blkHeight, uint bpp, int CurLevel)
        {
            ...
            {
                width_ = DIV_ROUND_UP(System.Math.Max(1, width >> mipLevel), blkWidth);
                height_ = DIV_ROUND_UP(System.Math.Max(1, height >> mipLevel), blkHeight);

                offset += width_ * height_ * bpp;
            }

            width_ = DIV_ROUND_UP(System.Math.Max(1, width >> CurLevel), blkWidth);
            height_ = DIV_ROUND_UP(System.Math.Max(1, height >> CurLevel), blkHeight);
