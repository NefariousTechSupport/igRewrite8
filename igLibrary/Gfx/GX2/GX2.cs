namespace igLibrary.Gfx.GX2Utils
{
    public class GX2
    {
               ...
               uint Width, uint Height, uint Depth, uint Format, uint swizzle, uint SurfaceDim, uint MipCount)
        {
            ...
            uint bpp = surfaceGetBitsPerPixel(Format) >> 3;
            int DepthLevel = 1;
            ...
            if (TileMode == 0)
                TileMode = getDefaultGX2TileMode(SurfaceDim, Width, Height, 1, Format, 0, 1);
            ...
            for (int mipLevel = 0; mipLevel < MipCount; mipLevel++)
            {
                ...
                //uint width_ = (uint)System.Math.Max(1, Width >> mipLevel);
                //uint height_ = (uint)System.Math.Max(1, Height >> mipLevel);
                ...
                uint width_ = System.Math.Max(1, Width >> mipLevel);
                uint height_ = System.Math.Max(1, Height >> mipLevel);

                if (mipLevel != 0)
                {
                    surfOut = getSurfaceInfo((GX2SurfaceFormat)Format, Width, Height, 1, 1, TileMode, 0, mipLevel);
                ...
                byte[] SwizzledData = GX2.swizzle(width_, height_, surfOut.depth, surfOut.height, Format, 0, 1, surfOut.tileMode, s,
            ...
            GX2Surface surf = new GX2Surface();
            surf.depth = Depth;
            surf.width = Width;
            surf.height = Height;
            surf.use = 1;
        ...
        {
            var surfOut = getSurfaceInfo((GX2SurfaceFormat)tex.format, tex.width, tex.height, 1, 1, tex.tileMode, 0, 0);
            uint imageSize = (uint)surfOut.surfSize;
            uint mipSize = 0;
            ...
            for (int mipLevel = 0; mipLevel < tex.numMips; mipLevel++)
            {
                if (mipLevel != 0)
                {
                    surfOut = getSurfaceInfo((GX2SurfaceFormat)tex.format, tex.width, tex.height, 1, 1, tex.tileMode, 0, mipLevel);
        ...
        public static byte[] Decode(GX2Surface tex, int ArrayIndex = -1, int MipIndex = -1, string DebugTextureName = "")
        {
            ...
            var ImageSurfInfo = getSurfaceInfo((GX2SurfaceFormat)tex.format, tex.width, tex.height, tex.depth, tex.dim, tex.tileMode, tex.aa, 0);
            ...
            for (int arrayLevel = 0; arrayLevel < tex.depth; arrayLevel++)
            {
                ...
                for (int mipLevel = 0; mipLevel < mipCount; mipLevel++)
                {
                    var MipSurfInfo = getSurfaceInfo((GX2SurfaceFormat)tex.format, tex.width, tex.height, tex.depth, tex.dim, tex.tileMode, tex.aa, mipLevel);

                    bool GetLevel = arrayLevel == ArrayIndex && mipLevel == MipIndex;

                    uint swizzle = tex.swizzle;

                    uint width_ = System.Math.Max(1, tex.width >> mipLevel);
                    uint height_ = System.Math.Max(1, tex.height >> mipLevel);
                    ...
                    if (GetLevel)
                    {
                        byte[] deswizzled = deswizzle(width_, height_, MipSurfInfo.depth, MipSurfInfo.height, tex.format, 0, tex.use,
                        MipSurfInfo.tileMode, swizzle, MipSurfInfo.pitch, MipSurfInfo.bpp, (uint)arrayLevel, 0, data);
        ...
        public static List<List<byte[]>> Decode(GX2Surface tex, string DebugTextureName = "")
        {
            ...
            var surfdEBUG = getSurfaceInfo((GX2SurfaceFormat)tex.format, tex.width, tex.height, tex.depth, tex.dim, tex.tileMode, tex.aa, 0);
            Debug(surfdEBUG);
            /*     Console.WriteLine("");
            ...
            var surfInfo = getSurfaceInfo((GX2SurfaceFormat)tex.format, tex.width, tex.height, tex.depth, tex.dim, tex.tileMode, tex.aa, 0);
            ...
            for (int arrayLevel = 0; arrayLevel < tex.depth; arrayLevel++)
            {
                ...
                for (int mipLevel = 0; mipLevel < mipCount; mipLevel++)
                {
                    ...
                    if (mipLevel != 0)
                    {
                        ...
                        mipOffset = tex.mipOffset[mipLevel - 1];
                        if (mipLevel == 1)
                            mipOffset -= surfInfo.sliceSize;

                        surfInfo = getSurfaceInfo((GX2SurfaceFormat)tex.format, tex.width, tex.height, tex.depth, tex.dim, tex.tileMode, tex.aa, mipLevel);
                    ...
                    byte[] deswizzled = deswizzle(width_, height_, surfInfo.depth, surfInfo.height, tex.format, 0, tex.use,
                    surfInfo.tileMode, swizzle, surfInfo.pitch, surfInfo.bpp, (uint)arrayLevel, 0, data);
                    //Create a copy and use that to remove uneeded data
                    byte[] result_ = new byte[size];
                    Array.Copy(deswizzled, 0, result_, 0, size);
                    mips.Add(result_);
    ...
    0x00, 0x01, 0x01, 0x03, 0x00, 0x01, 0x01, 0x03, 0x40, 0x01, 0x01, 0x03, 0x00, 0x01, 0x01, 0x03,
        ...
        private static uint computeSurfaceBankSwappedWidth(AddrTileMode tileMode, uint bpp, uint numSamples, uint pitch)
        {
            ...
            if (bytesPerSample != 0)
            {
                ...
                slicesPerTile = System.Math.Max(1, numSamples / samplesPerTile);
            }
            ...
            uint swapTiles = System.Math.Max(1, 128 / bpp);
            ...
            uint bankSwapWidth = System.Math.Min(swapMax, System.Math.Max(swapMin, swapWidth));
            ...
            uint numSlices, uint numSamples, uint isDepth, uint noRecursive)
        {
            ...
            {
                widthAlignFactor = System.Math.Max(1, 256 / microTileBytes);
            }
        ...
        private static uint computeSurfaceTileSlices(uint tileMode, uint bpp, uint numSamples)
        {
            ...
            if (bytePerSample != 0)
            {
                ...
                if (samplePerTile < numSamples)
                    tileSlices = System.Math.Max(1, numSamples / samplePerTile);
            }
        ...
        private static uint ComputeSurfaceInfoEx()
        {
            ...
            uint bpp = pIn.bpp;
            uint numSamples = System.Math.Max(1, pIn.numSamples);
        ...
        private static Tuple<uint, uint, uint> computeSurfaceAlignmentsLinear(uint tileMode, uint bpp, Flags flags)
        {
            ...
            else if (tileMode == 1)
            {
                ...
                baseAlign = 256;
                pitchAlign = System.Math.Max(0x40, pixelsPerPipeInterleave);
                heightAlign = 1;
            }
            else
            {
        ...
        private static void computeSurfaceInfo(surfaceIn aSurfIn, surfaceOut pSurfOut)
        {
            ...
            if (returnCode == 0)
            {
                ...
                {
                    pIn.width = System.Math.Max(1, pIn.width);
                    pIn.height = System.Math.Max(1, pIn.height);
                }
                else
                    returnCode = 3;
            ...
            uint pitch, uint height, uint numSlices, uint mipLevel, uint padDims, Flags flags)
        {
            ...
            else
            {
                ...
                pitchAlignFactor = System.Math.Max(1, 32 / bpp);
        ...
        private static Tuple<uint, uint, uint> computeSurfaceAlignmentsMicroTiled(uint tileMode, uint bpp, Flags flags, uint numSamples)
        {
            ...
            uint thickness = computeSurfaceThickness((AddrTileMode)tileMode);
            uint baseAlign = 256;
            uint pitchAlign = System.Math.Max(8, 256 / bpp / numSamples / thickness);
            uint heightAlign = 8;
        ...
        private static Tuple<uint, uint, uint, uint, uint> computeSurfaceAlignmentsMacroTiled(uint tileMode, uint bpp, Flags flags, uint numSamples)
        {
            ...
            uint pitchAlign = System.Math.Max(macroTileWidth, macroTileWidth * (256 / bpp / (8 * thickness) / numSamples));
            pitchAlign = adjustPitchAlignment(flags, pitchAlign);
            ...
            if (thickness == 1)
                baseAlign = System.Math.Max(macroTileBytes, numSamples * heightAlign * bpp * pitchAlign + 7 >> 3);
            else
                baseAlign = System.Math.Max(256, 4 * heightAlign * bpp * pitchAlign + 7 >> 3);
        ...
        private static uint adjustSurfaceInfo(uint elemMode, uint expandX, uint expandY, uint bpp, uint width, uint height)
        {
            ...
            if (width != 0 && height != 0)
            {
                if (expandX > 1 || expandY > 1)
                {
                    ...
                    pIn.width = System.Math.Max(1, widtha);
                    pIn.height = System.Math.Max(1, heighta);
                }
            }
        ...
        private static void computeMipLevel()
        {
            ...
            {
                width = System.Math.Max(1, pIn.width >> (int)pIn.mipLevel);
                height = System.Math.Max(1, pIn.height >> (int)pIn.mipLevel);
                slices = System.Math.Max(1, pIn.numSlices);

                if ((pIn.flags.value >> 4 & 1) == 0)
                    slices = System.Math.Max(1, slices >> (int)pIn.mipLevel);
        ...
        private static uint restoreSurfaceInfo(uint elemMode, uint expandX, uint expandY, uint bpp)
        {
            ...
            if (pOut.pixelPitch != 0 && pOut.pixelHeight != 0)
            {
                ...
                pOut.pixelPitch = System.Math.Max(1, width);
                pOut.pixelHeight = System.Math.Max(1, height);
            }
            if (bpp != 0)
            {
                switch (elemMode)
            ...
            if (49 <= pIn.format && pIn.format <= 55)
            {
                if (pIn.mipLevel != 0)
                {
                    ...
                    if ((pIn.flags.value >> 12 & 1) != 0)
                    {
                        ...
                        width = System.Math.Max(1, widtha);
                        height = System.Math.Max(1, heighta);
                        slices = System.Math.Max(1, slices);
                    }
        ...
        public static surfaceOut getSurfaceInfo(GX2SurfaceFormat surfaceFormat, uint surfaceWidth, uint surfaceHeight, uint surfaceDepth, uint surfaceDim, uint surfaceTileMode, uint surfaceAA, int level)
        {
            ...
            if (surfaceTileMode == 16)
            {
                ...
                width = ~(blockSize - 1) & System.Math.Max(1, surfaceWidth >> level) + blockSize - 1;
                ...
                {
                    pSurfOut.height = System.Math.Max(1, surfaceHeight >> level);
                    pSurfOut.depth = 1;
                }
                else if (dim == 2)
                {
                    pSurfOut.height = System.Math.Max(1, surfaceHeight >> level);
                    pSurfOut.depth = System.Math.Max(1, surfaceDepth >> level);
                }
                else if (dim == 3)
                {
                    pSurfOut.height = System.Math.Max(1, surfaceHeight >> level);
                    pSurfOut.depth = System.Math.Max(6, surfaceDepth);
                }
                else if (dim == 4)
                {
                    pSurfOut.height = 1;
            ...
            else
            {
                ...
                aSurfIn.width = System.Math.Max(1, surfaceWidth >> level);
                dim = surfaceDim;
                ...
                {
                    aSurfIn.height = System.Math.Max(1, surfaceHeight >> level);
                    aSurfIn.numSlices = 1;
                }
                else if (dim == 2)
                {
                    aSurfIn.height = System.Math.Max(1, surfaceHeight >> level);
                    aSurfIn.numSlices = System.Math.Max(1, surfaceDepth >> level);
                }
                else if (dim == 3)
                {
                    aSurfIn.height = System.Math.Max(1, surfaceHeight >> level);
                    aSurfIn.numSlices = System.Math.Max(6, surfaceDepth);
